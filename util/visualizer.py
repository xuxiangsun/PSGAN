...
class Visualizer():
    ...
    def print_current_acc(self, epoch, acc, size):
        """print current losses on console; also save the losses to the disk

        Parameters:
            epoch (int) -- current epoch
            acc (float) -- current accuracy
            size (int) -- size of images per-class
        """
        message = '(epoch: %d) ' % (epoch)

        message += 'Totalacc: %.3f%% ' % ((acc/size) * 100)

        print(message)  # print the message
        with open(self.acc_name, "a") as acc_file:
            acc_file.write('%s\n' % message)  # save the message


    def show_samples(self, name, x, epoch):
        for i in range(x.shape[0]):
            self.writer.add_image(name + '_{}'.format(i), ((x[i].detach().cpu().numpy() + 1.)/2.), epoch)