...
class scnet50model(BaseModel):

    def __init__(self, opt):
        BaseModel.__init__(self, opt)
        self.opt = opt
        self.target_model = scnet50_v1d(pretrained=False)
        self.target_model.load_state_dict(torch.load('./pretrained/scnet50_v1d-4109d1e1.pth'))
        self.num_fits = self.target_model.fc.in_features
        self.target_model.fc = nn.Linear(self.num_fits, opt.classes)
        self.model_names = ['scnet50']
        ...
        self.netscnet50 = self.target_model
    ...
    def forward(self):
        self.logits = self.netscnet50(self.imgs)
