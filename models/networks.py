...
def define_G(input_nc, output_nc, cla, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        ...
        net = PSGenerator(input_nc, output_nc, cla, ngf, norm_layer=norm_layer)
    elif netG == 'unet_128':
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, cla=None, gpu_ids=[]):
    ...
    elif netD == 'psgan':  # proposed architecture of discriminator by PSGAN
        net = PSDiscriminator(input_nc, cla, ndf, n_layers=n_layers_D, norm_layer=norm_layer)
...
class PSGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, cla, ngf=64, norm_layer=nn.BatchNorm2d):
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if i == 0:
                up = [nn.ConvTranspose2d(ngf * mult + cla, int(ngf * mult / 2),
    ...
    def forward(self, input, label):
        """Standard forward"""
        l = label.view(label.size(0), label.size(1), 1, 1).expand(label.size(0), label.size(1), input.size(2), input.size(3))
        cinput = torch.cat([input, l], 1)
        down = self.down(cinput)
        l = label.view(label.size(0), label.size(1), 1, 1).expand(label.size(0), label.size(1), down.size(2), down.size(3))
...
class NLayerDiscriminator(nn.Module):
    ...
    def forward(self, input):
        """Standard forward."""
        return self.model(input), torch.cat([self.feat1(input), self.feat2(input), self.feat3(input)], 1)
...
class PSDiscriminator(nn.Module):
    ...
    def forward(self, input):
        """Standard forward."""
        x = self.model(input)
        return self.fc(x.view(x.size(0), -1)), torch.cat([self.feat1(input)/8., self.feat2(input)/4., self.feat3(input)], 1)
